...
import { AuthCallbackKind, parseSupabaseAuthError, parseSupabasePkceRedirect } from '../../services/supabase/authRedirect'
...
function redirectErrorMessage(raw: string | null): string {
  if (!raw) return 'O link de autenticação é inválido ou não contém um código utilizável.'
  if (/expired|otp_expired/i.test(raw)) return 'Este link expirou. Solicite um novo e tente novamente.'
  return mapAuthError(raw)
}

export function AuthPkceCallback({ kind }: { kind: AuthCallbackKind }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const rawError = parseSupabaseAuthError(url, kind)
      if (rawError) {
        if (active) {
      ...
      const redirect = parseSupabasePkceRedirect(url, kind)
      ...
      // O code verifier foi criado pelo supabase-js e persistido no mesmo aparelho
      // quando signUp/resetPasswordForEmail iniciou o fluxo PKCE.
  ...
  const savePassword = async () => {
    ...
    // Recovery cria sessão autenticada. Depois de alterar a senha, encerramos a
    // sessão e apagamos o espelho local para exigir um login explícito novo.
    await signOut()
    setPassword('')
    setConfirmPassword('')
