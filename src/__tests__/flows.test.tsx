...
describe('fluxo canônico local-first', () => {
  it('registrar medição → persiste no espelho, agenda sync e aparece no calendário', async () => {
    ...
    // outbox: o provider dispara sync no mount; após syncNow a fila é drenada sem duplicar a linha
    await h.core.sync.syncNow()
    const serverRows = [...(h.server.get('glucose_measurements')?.values() ?? [])]
    expect(serverRows).toHaveLength(1) // 1 operação aplicada, não 2 (idempotência por linha)
    expect(await h.db.select('sync_outbox')).toHaveLength(0)
    expect(await h.core.measurements.listByDay('2026-09-09')).toHaveLength(1)

    setParams({})
    const cal = h.renderApp(<Calendar />)
    await waitFor(() => expect(cal.toJSON()).not.toBeNull())
    // contagem do dia atual no calendário
  ...
  it('meta configurada: valor fora da faixa vira TEXTO de aviso — não cor isolada', async () => {
    ...
    await h.core.measurements.create({ valueText: '250', unit: 'mg/dL', localDate: '2026-09-09', localTime: '07:00', context: 'fasting', note: '' })
    const rows = await h.core.measurements.listByDay('2026-09-09')
